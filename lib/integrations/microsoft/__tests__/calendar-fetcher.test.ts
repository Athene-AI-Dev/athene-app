...
describe('calendar-fetcher', () => {
  ...
  it('fetchEvents should call correct endpoint with ISO strings', async () => {
    ...
    await fetchEvents('conn-123', start, end)
  ...
  it('createEvent should call events endpoint with POST', async () => {
    ...
    await createEvent('conn-123', event)
    expect(graphClient.graphFetch).toHaveBeenCalledWith('conn-123', '/me/events', expect.objectContaining({
        method: 'POST',
  ...
  it('findFreeSlots should format duration as ISO 8601 PTnM', async () => {
    await findFreeSlots('conn-123', ['test@example.com'], 45)
    expect(graphClient.graphFetch).toHaveBeenCalledWith('conn-123', '/me/findMeetingTimes', expect.objectContaining({
