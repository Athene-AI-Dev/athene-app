...
describe('outlook-fetcher', () => {
  ...
  it('fetchUnreadEmails should call correct endpoint', async () => {
    ...
    const emails = await fetchUnreadEmails('conn-123')
    expect(graphClient.graphFetch).toHaveBeenCalledWith('conn-123', expect.stringContaining('/me/messages?$filter=isRead eq false'))
  ...
  it('fetchEmailBody should return content', async () => {
    ...
    const body = await fetchEmailBody('conn-123', 'm1')
    expect(body).toBe('hello world')
  })

  it('sendEmail should call sendMail endpoint with POST', async () => {
    await sendEmail('conn-123', { 
        subject: 'hi', 
        body: { contentType: 'Text', content: 'test' }, 
        toRecipients: [{ emailAddress: { address: 'test@example.com' } }] 
    })
    expect(graphClient.graphFetch).toHaveBeenCalledWith('conn-123', '/me/sendMail', expect.objectContaining({ 
        method: 'POST',
