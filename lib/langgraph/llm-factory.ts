...
/**
 * Fetch all active BYOK key rows for an org.
 * Returns empty array if none exist or on query error.
 */
async function fetchByokRows(
...
export async function resolveModelClient(
  ...
  // Attempt BYOK — fetch all active keys for the org in one query
  const byokRows = await fetchByokRows(supabase, orgId);

  if (byokRows.length > 0) {
    const providers: ModelProvider[] = ["anthropic", "openai", "google"];

    for (const provider of providers) {
      const row = byokRows.find((r) => r.provider === provider);
      if (!row) continue;
