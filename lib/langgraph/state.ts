...
//
// Canonical schema defined in ATH-37 (Prajwal).
// Extended here with hop_count + reasoning for ATH-34 supervisor.
//
// Design rules:
...
export const AtheneStateAnnotation = Annotation.Root({
  ...
  /** Active access_grant row id when user has BI cross-dept grants */
  ...
  is_cross_dept_query: Annotation<boolean>({ reducer: lw, default: () => false }),
  /** Number of supervisor→worker round-trips in this run */
  hop_count: Annotation<number>({ reducer: lw, default: () => 0 }),
  /** Latest supervisor reasoning — streamed to frontend via state updates */
  reasoning: Annotation<string>({ reducer: lw, default: () => "" }),

  // --- Retrieved context (ephemeral — cleared after synthesis) ---

  retrieved_chunks: Annotation<RetrievedChunk[]>({ reducer: lw, default: () => [] }),
  ...
  awaiting_approval: Annotation<boolean>({ reducer: lw, default: () => false }),
  ...
  cited_sources: Annotation<CitedSource[]>({ reducer: lw, default: () => [] }),
});
...
/** Alias for backward compatibility with existing node files */
export type AtheneStateType = AtheneState;
