...
export default clerkMiddleware(async (auth, request) => {
  ...
  if (userId && orgId) {
    try {
      ...
      if (access.internal_user_id) requestHeaders.set("x-current-user-id", access.internal_user_id);
      if (access.role) requestHeaders.set("x-current-user-role", access.role);
